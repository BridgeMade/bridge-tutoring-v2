...
export function assessmentBookingConfirmationEmail(data: {
  parentName: string;
  childName: string;
  gradeLevel: string;
  subjects: string[];
  ...
  const formatLabel = data.lessonFormat === "online" ? "Online" : "In-person";
    ...
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;">
        ...
        <tr>
          <td style="padding:40px 40px 32px;">
            <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#171717;">
              Assessment requested, ${data.parentName}!
            </h1>
            <p style="margin:0 0 24px;font-size:15px;color:#525252;line-height:1.6;">
              Thanks for confirming ${data.childName}'s details. We're matching a tutor against
              one of your three time options below and will confirm which one shortly — no
              further back-and-forth needed on your side.
            </p>
              ...
              <tr>
                <td>
                  ...
                  <p style="margin:8px 0 0;font-size:14px;color:#404040;line-height:1.8;">
                    <strong>Child:</strong> ${data.childName}<br>
                    <strong>Grade:</strong> ${data.gradeLevel}<br>
                    <strong>Subject:</strong> ${data.subjects.join(", ")}<br>
                    <strong>Format:</strong> ${formatLabel}<br>
                    ${data.area ? `<strong>Area:</strong> ${data.area}<br>` : ""}
                    ${data.address ? `<strong>Address:</strong> ${data.address}<br>` : ""}
                    <strong>Your date/time options:</strong><br>
                    ${timeOptionsHtml}
                  </p>
                </td>
              </tr>
            ...
            <p style="margin:0 0 16px;font-size:15px;color:#525252;line-height:1.6;">
              The assessment is free and takes about 45 minutes, with no obligation to continue
              afterwards. If none of your options end up working, just reply to this email or
